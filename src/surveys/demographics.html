<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta http-equiv="X-UA-Compatible" content="ie=edge">
    <title>Document</title>
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap@4.0.0/dist/css/bootstrap.min.css" integrity="sha384-Gn5384xqQ1aoWXA+058RXPxPg6fy4IWvTNh0E263XmFcJlSAwiGgFAW/dAiS6JXm" crossorigin="anonymous">
    <link rel="stylesheet" href="../demographics.css">
    <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.6.0/jquery.min.js"></script>
    <script src="https://cdn.jsdelivr.net/npm/popper.js@1.12.9/dist/umd/popper.min.js" integrity="sha384-ApNbgh9B+Y1QKtv3Rn7W3mgPxhU9K/ScQsAP7hUibX39j7fakFPskvXusvfa0b4Q" crossorigin="anonymous"></script>
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@4.0.0/dist/js/bootstrap.min.js" integrity="sha384-JZR6Spejh4U02d8jOt6vLEHfe/JQGiRRSQQxSfFWpi1MquVdAyjUar5+76PVCmYl" crossorigin="anonymous"></script>
</head>
<body>
    <div class = "wholeForm">
        <p class = "title">Welcome to this Codetrics Experiment!</p>
        <p class = "postTitle">Please fill out this demographics form before you start the experiment.</p>
        <hr>
        <div class = "label">What is your gender identity?</div>
        <div class="form-check">
            <input class="form-check-input" name="genderRadio" type="radio" id="male" value="option1" checked>
            <label class="form-check-label" for="male">
                Male
            </label>
        </div>
        <div class="form-check">
            <input class="form-check-input" name="genderRadio" type="radio" id="female" value="option2">
            <label class="form-check-label" for="female">
                Female
            </label>
        </div>
        <div class="form-check">
            <input class="form-check-input" name="genderRadio" type="radio" id="nonBinary" value="option3">
            <label class="form-check-label" for="nonBinary">
                Non-binary / third gender
            </label>
        </div>
        <div class="form-check">
            <input class="form-check-input" name="genderRadio" type="radio" id="noSelection" value="option3">
            <label class="form-check-label" for="noSelection">
                Prefer not to say
            </label>
        </div>

        <br>

        <div class = "label">How would you rate your overall programming expertise?</div>
        <div class="form-check">
            <input class="form-check-input" name="experienceRadio" type="radio" id="none" value="option1" checked>
            <label class="form-check-label" for="none">
                None
            </label>
        </div>
        <div class="form-check">
            <input class="form-check-input" name="experienceRadio" type="radio" id="low" value="option2">
            <label class="form-check-label" for="low">
                Low
            </label>
        </div>
        <div class="form-check">
            <input class="form-check-input" name="experienceRadio" type="radio" id="medium" value="option3">
            <label class="form-check-label" for="medium">
                Medium
            </label>
        </div>
        <div class="form-check">
            <input class="form-check-input" name="experienceRadio" type="radio" id="high" value="option3">
            <label class="form-check-label" for="high">
                High
            </label>
        </div>
        <br>
        <div class = "label">How long have you been programming in years? </div>
        <div id = "numLabels"></div>
        <input class = "range" type="range" value="50" min="0" max="100" step = "5"oninput="this.nextElementSibling.value = this.value">
        <output>50</output>

        <br>

        <div class = "label">On average how often do you program (within the past year)?</div>
        <div class="form-check">
            <input class="form-check-input" name="frequencyRadio" type="radio" id="oneOrLess" value="option1" checked>
            <label class="form-check-label" for="oneOrLess">
                1 hour per week or less
            </label>
        </div>
        <div class="form-check">
            <input class="form-check-input" name="frequencyRadio" type="radio" id="oneToFive" value="option2">
            <label class="form-check-label" for="oneToFive">
                between 1 and 5 hours per week
            </label>
        </div>
        <div class="form-check">
            <input class="form-check-input" name="frequencyRadio" type="radio" id="fiveToTen" value="option3">
            <label class="form-check-label" for="fiveToTen">
                between 5 and 10 hours per week
            </label>
        </div>
        <div class="form-check">
            <input class="form-check-input" name="frequencyRadio" type="radio" id="tenToTwenty" value="option3">
            <label class="form-check-label" for="tenToTwenty">
                between 10 and 20 hours per week
            </label>
        </div>
        <div class="form-check">
            <input class="form-check-input" name="frequencyRadio" type="radio" id="moreThanTwenty" value="option3">
            <label class="form-check-label" for="moreThanTwenty">
                more than 20 hours per week
            </label>
        </div>
        <input class = "btn submit" type = "submit">
    </div>
</body>
<script src = "/scripts/js/demographics.js"></script>
<script>
        container = document.getElementById("numLabels")
        console.log(container)
        for (var i = 0; i <101; i+=5){
            div = document.createElement("div")
            div.innerHTML = i
            div.setAttribute("class", "num")
            container.appendChild(div)
        }
</script>
</html>